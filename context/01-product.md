...
| **Report an issue** (in-app → DB + email) | ✅ | ✅ | ✅ | 2026-09 beta-launch feature. Desktop: Settings → Support (shared `flume_html()`, so Mac = Windows). Mobile: Settings → About → Report an issue (`ReportIssueScreen`). All platforms call the `report-issue` Edge Function → `issue_reports` table + best-effort Resend email to the founder — see `03-features.md` §Report an issue |
