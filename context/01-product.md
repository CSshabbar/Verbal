...
| **Linux desktop** | `whisperflow/app/linux_*.py` | `pystray` (AppIndicator) tray + tkinter overlay + pywebview (GTK/WebKit2); clipboard + XTEST paste; shares the core pipeline | Newest and least complete. Core loop (record → transcribe → AI-clean → paste) + sync work. **On Wayland the in-process global hotkey cannot work** — the desktop owns the key and runs `verbal --toggle` over an IPC socket (`--install-hotkey`). Renders the legacy inline `SharedDashboard._html()`, not `flume_html()`, so dictionary/pairing/recordings/onboarding/sign-in UI is unreachable. No recordings persistence, auto-learn, file-tagging, meetings or transform |

All four share **one Supabase backend** (project `ovpcthjingugwvpxlsna`) — see `04-data-model.md`. The
...
called out in the Notes column rather than adding a separate Android matrix column.

## Feature availability matrix

| Feature | macOS | iOS | Windows | Linux | Notes |
|---|:--:|:--:|:--:|:--:|---|
...
| Custom dictionary (vocab bias + replacement rules) | ✅ | ✅ | ✅ | ⚠️ | Synced, one row/user |
| **Snippets** (spoken trigger → text expansion) | ✅ | ✅ | ✅ | ⚠️ | On the `dictionary` row; longest-first, single-pass. Windows expands in-dictation via `dictionary.apply_snippets` |
...
| Notes (voice-first, synced) | ✅ | ✅ | ✅ | ⚠️ | Supabase `notes` table. **v2:** search, auto-title, structure/checklists, audio linkage, raw+formatted, conflict-pair sync; 4 flags (`notes_search/autotitle/structure_detection/audio_linkage_enabled`, default on). Windows uses the shared `DashboardApi` + Flume notes UI |
...
```
Verbal/
├── whisperflow/         # macOS + Windows + Linux desktop app (Python).  app/*.py
