...
| **Windows desktop** | `whisperflow/app/win_*.py` | `pystray` tray + `pynput` + pywebview (WebView2), shares core modules **and now the same Flume UI** | Near parity — identical Flume dashboard (`flume_html()`); auth/dictation/sync/notes/canvas/meetings/auto-learn/file-tagging/transform wired. Overlay stays tkinter (WebView2 cannot do a sticker pill). |
...
| **Auto-learn** dictionary from corrections | ✅ | ❌ | ✅ | macOS AX read-back + cream pill. Windows UIA read-back (`win_editwatch.py`) + tkinter confirm pill (`win_autolearn_widget.py`) |
| **File tagging** (`@name.ext` in IDEs) | ✅ | ❌ | ✅ | macOS Accessibility. Windows UI Automation harvest (`win_ax.py`) — `windows_specs/W8-filetags-uia.md` |
...
| Recording overlay / floating HUD | ✅ | ❌ | ✅ | Desktop pill; iOS uses a modal screen. Windows is a tkinter + PIL sticker (`win_overlay.py`) — WebView2 cannot do per-pixel transparency, so `overlay_html()` is Mac-only on purpose (do not implement W3 as written) |
...
| **Transform — inline** (“…so Flume, make it formal”) | ✅ | ❌ (planned) | ✅ | Trailing-instruction gate + LLM rewrite before paste; default OFF (`transform_enabled`) |
| **Transform — selection** (⌘⇧T / Ctrl+Shift+T → preview → replace) | ✅ | ✅ | ✅ | Desktop: clipboard-captured selection, cream pill (Improvise / spoken / typed), preview + undo. Windows host is `win_transform_widget.py`. Mobile (iOS + Android via the shared Expo codebase, same no-separate-Android-column caveat as the clipboard row): a dedicated Transform button on the custom keyboard reads the host field's selection (`selectedText`/`getSelectedText`), same Improvise/typed/spoken instruction paths and prompts, preview before replace; "undo" is a soft delete-and-reinsert (no OS-level undo API on mobile). Default OFF (`transformEnabled`, Settings → Keyboard), matching desktop's opt-in posture |
...
| **Report an issue** (in-app → DB + email, optional screenshot) | ✅ | ✅ | ✅ | 2026-09 beta-launch feature. Desktop: Settings → Support (shared `flume_html()`, so Mac = Windows; native file picker for the screenshot). Mobile: Settings → About → Report an issue (`ReportIssueScreen`; `expo-image-picker`). All platforms call the `report-issue` Edge Function → `issue_reports` table (+ private `issue-screenshots` bucket) + best-effort Resend email to the founder with the screenshot attached — see `03-features.md` §Report an issue |
| **Beta signup form** (website, not in-app) | — | — | — | 2026-09 beta-launch feature. `idiaz.io/flume/beta.html` — THE public link for beta distribution: name+email → instant download buttons + branded welcome email; founder notified per signup. `beta-signup` Edge Function → `beta_signups` table — see `03-features.md` §Beta signup |
